...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.4.29-cosigntest.0`
...
`ghcr.io/astral-sh/uv:0.4.29-cosigntest.0-alpine`.
...
```dockerfile
COPY --from=ghcr.io/astral-sh/uv:0.4.29-cosigntest.0 /uv /uvx /bin/
```

Or, with the installer:

```dockerfile
ADD https://astral.sh/uv/0.4.29-cosigntest.0/install.sh /uv-installer.sh
```
